...
public class TunableArmFeedforward implements ProtobufSerializable, StructSerializable {
     ...
     */
    public TunableArmFeedforward(DoubleSupplier ks, DoubleSupplier kg, DoubleSupplier kv, DoubleSupplier ka, double dtSeconds) {
        this.ks = ks;
        this.kg = kg;
        this.kv = kv;
        this.ka = ka;
        if (kv.getAsDouble() < 0.0) {
            throw new IllegalArgumentException("kv must be a non-negative number, got " + kv + "!");
        }
        if (ka.getAsDouble() < 0.0) {
     ...
     * @throws IllegalArgumentException for ka &lt; zero.
     */
    public TunableArmFeedforward(DoubleSupplier ks, DoubleSupplier kg, DoubleSupplier kv, DoubleSupplier ka) {
     ...
     * @throws IllegalArgumentException for kv &lt; zero.
     */
    public TunableArmFeedforward(DoubleSupplier ks, DoubleSupplier kg, DoubleSupplier kv) {
        this(ks, kg, kv, () -> 0);
    }
    ...
    public double calculate(
        ...
        return ArmFeedforwardJNI.calculate(
                ks.getAsDouble(), kv.getAsDouble(), ka.getAsDouble(), kg.getAsDouble(), currentAngle, currentVelocity, nextVelocity, dt);
    }
    ...
    public double calculateWithVelocities(
        ...
        return ArmFeedforwardJNI.calculate(
                ks.getAsDouble(), kv.getAsDouble(), ka.getAsDouble(), kg.getAsDouble(), currentAngle, currentVelocity, nextVelocity, m_dt);
    }
    ...
    public double maxAchievableVelocity(double maxVoltage, double angle, double acceleration) {
        ...
        return (maxVoltage - ks.getAsDouble() - Math.cos(angle) * kg.getAsDouble() - acceleration * ka.getAsDouble()) / kv.getAsDouble();
    }
    ...
    public double maxAchievableAcceleration(double maxVoltage, double angle, double velocity) {
        return (maxVoltage - ks.getAsDouble() * Math.signum(velocity) - Math.cos(angle) * kg.getAsDouble() - velocity * kv.getAsDouble()) / ka.getAsDouble();
    }
