 ...
 */
public class TunableTrapezoidProfile {
  // The direction of the profile, either 1 for forwards or -1 for inverted
  private int m_direction;

  private final TunableConstraints m_constraints;
  ...
  /** Profile constraints. */
  public static class TunableConstraints {
    /** Maximum velocity. */
    public final DoubleSupplier maxVelocity;

    /** Maximum acceleration. */
    public final DoubleSupplier maxAcceleration;
     ...
     */
    public TunableConstraints(DoubleSupplier maxVelocity, DoubleSupplier maxAcceleration) {
   ...
   */
  public TunableTrapezoidProfile(TunableConstraints constraints) {
  ...
  public State calculate(double t, State current, State goal) {
    ...
    if (m_current.velocity > m_constraints.maxVelocity.getAsDouble()) {
      m_current.velocity = m_constraints.maxVelocity.getAsDouble();
    }

    // Deal with a possibly truncated motion profile (with nonzero initial or
    // final velocity) by calculating the parameters as if the profile began and
    // ended at zero velocity
    double cutoffBegin = m_current.velocity / m_constraints.maxAcceleration.getAsDouble();
    double cutoffDistBegin =
        cutoffBegin * cutoffBegin * m_constraints.maxAcceleration.getAsDouble() / 2.0;

    double cutoffEnd = goal.velocity / m_constraints.maxAcceleration.getAsDouble();
    double cutoffDistEnd =
        cutoffEnd * cutoffEnd * m_constraints.maxAcceleration.getAsDouble() / 2.0;
    ...
    double accelerationTime =
        m_constraints.maxVelocity.getAsDouble() / m_constraints.maxAcceleration.getAsDouble();

    double fullSpeedDist =
        fullTrapezoidDist
            - accelerationTime * accelerationTime * m_constraints.maxAcceleration.getAsDouble();

    // Handle the case where the profile never reaches full speed
    if (fullSpeedDist < 0) {
      accelerationTime = Math.sqrt(fullTrapezoidDist / m_constraints.maxAcceleration.getAsDouble());
      fullSpeedDist = 0;
    }

    m_endAccel = accelerationTime - cutoffBegin;
    m_endFullSpeed = m_endAccel + fullSpeedDist / m_constraints.maxVelocity.getAsDouble();
    m_endDecel = m_endFullSpeed + accelerationTime - cutoffEnd;
    State result = new State(m_current.position, m_current.velocity);

    if (t < m_endAccel) {
      result.velocity += t * m_constraints.maxAcceleration.getAsDouble();
      result.position +=
          (m_current.velocity + t * m_constraints.maxAcceleration.getAsDouble() / 2.0) * t;
    } else if (t < m_endFullSpeed) {
      result.velocity = m_constraints.maxVelocity.getAsDouble();
      result.position +=
          (m_current.velocity + m_endAccel * m_constraints.maxAcceleration.getAsDouble() / 2.0)
                  * m_endAccel
              + m_constraints.maxVelocity.getAsDouble() * (t - m_endAccel);
    } else if (t <= m_endDecel) {
      result.velocity =
          goal.velocity + (m_endDecel - t) * m_constraints.maxAcceleration.getAsDouble();
      double timeLeft = m_endDecel - t;
      result.position =
          goal.position
              - (goal.velocity + timeLeft * m_constraints.maxAcceleration.getAsDouble() / 2.0)
                  * timeLeft;
    } else {
      result = goal;
    }
  ...
  public double timeLeftUntil(double target) {
    ...
    final double acceleration = m_constraints.maxAcceleration.getAsDouble();
    final double deceleration = -m_constraints.maxAcceleration.getAsDouble();
    ...
    double fullSpeedDist = m_constraints.maxVelocity.getAsDouble() * endFullSpeed;
    double decelDist;
    ...
    double fullSpeedTime = fullSpeedDist / m_constraints.maxVelocity.getAsDouble();
